    ...
    try:
        ...
        for col in required_cols:
            if col not in df.columns:
                print(
                    f"Error: Column '{col}' not found in Excel. Found: {list(df.columns)}"
                )
                return
        ...
        for index, row in df.iterrows():
            ...
            # Check if exists
            student = (
                db.query(models.Student).filter(models.Student.code == code).first()
            )
        ...
        db.commit()
        print(
            f"Success! Created {count} new students. Updated {updated} existing students."
        )
