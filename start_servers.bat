...
start "DB-Server-Node" cmd /k "cd /d c:\Sistema_reportes_academicos && npm start"

echo [2/2] Starting Main Application (Python on Port 8001)...
start "App-Server-Python" cmd /k "cd /d c:\Sistema_acompañamiento_integral && set PGCLIENTENCODING=UTF8 && set LC_MESSAGES=Spanish_Colombia.1252 && python -m src.main"
